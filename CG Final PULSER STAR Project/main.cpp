...
CDrop * ParticlesDrops;
SVertex * ParticlesVertices;
GLint Steps = 4;
GLint RaysPerStep =7;
GLint DropsPerRay = 800;
GLfloat DropsComplete = Steps * RaysPerStep * DropsPerRay;
GLfloat AngleOfDeepestStep = 1000;
GLfloat AccFactor = 0.004;
...
void InitParticles(void)
{
	...
	ParticlesDrops = new CDrop [ (int)DropsComplete ];
	ParticlesVertices = new SVertex [ (int)DropsComplete ];
	SVertex NewSpeed;
	...
	for (k = 0; k <Steps; k++)
	{
		for (j = 0; j < RaysPerStep; j++)
		{
			for (i = 0; i < DropsPerRay; i++)
			{
				...
				ParticlesDrops[i+j*DropsPerRay+k*DropsPerRay*RaysPerStep].SetConstantSpeed ( NewSpeed );
				ParticlesDrops[i+j*DropsPerRay+k*DropsPerRay*RaysPerStep].SetAccFactor (DropAccFactor);
				ParticlesDrops[i+j*DropsPerRay+k*DropsPerRay*RaysPerStep].SetTime(TimeNeeded * i / DropsPerRay);
			}
		}
	}
...
if(flag==0)
	...
	{
		ParticlesDrops[i].GetNewPosition(&ParticlesVertices[i]);
	}
...
/////////////////////Main Star////////////////////////
	glPushMatrix();
        ...
        glTranslatef(0,0,0);
        glRotatef(x+=1,0.5,1,0);
        glutSolidSphere(.61,8,5);
...
glPushMatrix();
    ...
    glColor4f(64.0f, 0.0f, 0.0f, 0.5f);
    glutSolidSphere(0.68, 50, 50);
    glDisable(GL_BLEND);
glPopMatrix();
...
void menu1()
{
	...
	DrawTextXY(-2.7,3.5,0.0,0.003,"     PULSER ");
...
void menu2()
{
	...
	DrawTextXY(-1.25,2.1,0.0,0.001,"  Move Near  ->  + ");
	DrawTextXY(-1.25,1.9,0.0,0.001,"  Move Away  ->  - ");
	DrawTextXY(-1.25,1.5,0.0,0.001,"  Move Up    ->  W ");
	DrawTextXY(-1.25,1.3,0.0,0.001,"  Move Down  ->  S ");
	DrawTextXY(-1.25,0.9,0.0,0.001,"  Rotate Up  ->  A ");
	DrawTextXY(-1.25,0.7,0.0,0.001,"  Rotate Down->  D ");
...
void cover()
{
	...
	glColor3f(0.6,0.8,0.7);
	DrawTextXY(-1.75,3,0.0,0.0014,"    NASA | Pulsar Star ");

	glColor3f(0.7,0.6,0.1);
	DrawTextXY(-3.25,1.5,0.0,0.001,"Submitted by : Ariful Islam Fahim");


	glColor3f(0.7,0.8,0.6);
	DrawTextXY(-3.25,0.95,0.0,0.001,"ID: 16-32651-3 ");



	glColor3f(0.7,0.6,0.1);
	DrawTextXY(-2.0,0,0.0,0.0007,"Under the guidance of : AZM EHTESHAM CHOWDHURY ");
	glColor3f(1.0,0.8,0.4);

	DrawTextXY(-1,-.5,0.0,0.0007,"     Designation ");
	DrawTextXY(-1,-.7,0.0,0.001,"     AIUB");
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("NASA | Pulsar Star");
